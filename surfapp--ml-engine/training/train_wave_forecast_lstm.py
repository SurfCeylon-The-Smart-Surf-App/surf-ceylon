...
def scale_data(X, y):
    ...
    print(
        f"  X range before scaling: [{X_reshaped.min():.2f}, {X_reshaped.max():.2f}]")
    print(
        f"  y range before scaling: [{y_reshaped.min():.2f}, {y_reshaped.max():.2f}]")
    ...
    print(
        f"  X range after scaling: [{X_scaled_flat.min():.2f}, {X_scaled_flat.max():.2f}]")
    print(
        f"  y range after scaling: [{y_scaled_flat.min():.2f}, {y_scaled_flat.max():.2f}]")

    # Reshape back
    X_scaled = X_scaled_flat.reshape(n_samples, n_timesteps_in, n_features_in)
    y_scaled = y_scaled_flat.reshape(
        n_samples, n_timesteps_out, n_features_out)
...
def evaluate_model(model, X_test, y_test, scaler_y):
    ...
    for i, name in enumerate(FEATURE_NAMES):
        ...
        mape = np.mean(
            np.abs((y_pred[:, :, i] - y_true[:, :, i]) / (y_true[:, :, i] + epsilon))) * 100
...
def main():
    ...
    for i, name in enumerate(FEATURE_NAMES):
        ...
        print(
            f"  {name:25s}: Pred={pred_avg:7.3f}, True={true_avg:7.3f}, Error={error:7.3f}")
