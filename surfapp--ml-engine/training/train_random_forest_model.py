...
def _get_average_from_sources(source_dict):
    ...
    if not source_dict:
        return None
    valid_values = [v for v in source_dict.values()
    ...
    for filepath in files:
        try:
            ...
            if 'hours' not in data or not data['hours']:
                print(
                    f"Warning: No 'hours' data in {filepath}", file=sys.stderr)
                continue
            ...
            print(
                f"  Loaded {len([r for r in all_records])} records from {filepath}", file=sys.stderr)

        except FileNotFoundError:
            print(
                f"Warning: {filepath} not found, skipping...", file=sys.stderr)
            continue
...
def preprocess_data(df):
    ...
    for col in all_columns:
        if col in df.columns:
            ...
            if missing_pct > 0:
                print(
                    f"  {col}: {missing_pct:.2f}% missing - filling with median", file=sys.stderr)
    ...
    for col in all_columns:
        if col in df.columns:
            ...
            if outliers > 0:
                print(f"  {col}: Removing {outliers} outliers",
                      file=sys.stderr)
                df = df[~outliers_mask]
...
def train_model(df):
    ...
    if missing_features or missing_targets:
        print(
            f"Error: Missing columns. Features: {missing_features}, Targets: {missing_targets}", file=sys.stderr)
        return
