...
MODEL_PATH = os.path.join(os.path.dirname(
    __file__), "..", "models", "ar_surfboard", "enhanced_ar_model.joblib")
...
class ARSurfboardRecommender:
    ...
    def __init__(self, model_path):
        ...
        try:
            ...
            print(
                f"[DEBUG] Model keys: {list(data.keys()) if isinstance(data, dict) else 'N/A'}", file=sys.stderr)
            ...
            print(
                f"[ERROR] Failed to load model: {type(e).__name__}: {str(e)}", file=sys.stderr)
            import traceback
    ...
    def predict(self, height_cm, weight_kg, age, experience_level, gender='Male'):
        ...
        try:
            ...
            board_volume = self.calculate_ideal_volume(
                weight_kg, height_cm, experience_level)
            board_length_m = self.calculate_board_length(
                height_cm, weight_kg, experience_level)

            # ML-based wave height prediction
            exp_encoded = self.label_encoders['surfer_experience'].transform(
                [experience_level])[0]
            gender_encoded = self.label_encoders['surfer_gender'].transform([gender])[
                0]
            bmi = weight_kg / ((height_cm / 100) ** 2)
            height_weight_ratio = height_cm / weight_kg

            features = np.array(
                [[height_cm, weight_kg, age, exp_encoded, gender_encoded, bmi, height_weight_ratio]])
            ...
            tips = self._generate_tips(
                experience_level, board_length_ft, wave_height, bmi)
    ...
    def _generate_tips(self, experience, board_length_ft, wave_height, bmi):
        ...
        if board_length_ft >= 7:
            tips.append(
                {"type": "equipment", "text": "Longboard setup - Maximum stability for learning"})
        elif board_length_ft >= 6.5:
            tips.append(
                {"type": "equipment", "text": "Fun board - Great balance of control and maneuverability"})
        else:
            tips.append(
                {"type": "equipment", "text": "Shortboard - High performance and quick turning"})

        # Wave conditions
        tips.append(
            {"type": "conditions", "text": f"Optimized for {wave_height:.1f}ft waves"})

        # Skill-based advice
        if experience in ['First-timer', 'Beginner']:
            tips.append(
                {"type": "technique", "text": "Focus on: Paddling, pop-up timing, and balance"})
        elif experience == 'Intermediate':
            tips.append(
                {"type": "technique", "text": "Practice: Bottom turns, cutbacks, and wave reading"})
        else:
            tips.append(
                {"type": "technique", "text": "Master: Advanced maneuvers and barrel riding"})
