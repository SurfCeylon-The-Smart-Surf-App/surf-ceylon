...
try:
    ...
    spots_file_path = os.path.join(os.path.dirname(
        __file__), '..', '..', 'SurfApp--frontend', 'data', 'surf_spots.json')
    ...
    SURF_SPOTS = [
        {'id': '1', 'name': 'Weligama', 'region': 'South Coast',
            'coords': [80.4264, 5.9721]},
        {'id': '13', 'name': 'Arugam Bay',
            'region': 'East Coast', 'coords': [81.8293, 6.8434]}
    ]
...
def run_ml_prediction(features):
    ...
    Returns:
        dict: Prediction with waveHeight, windSpeed, windDirection, tide (wavePeriod from API input)
    """
    try:
        ...
        tide_status = 'High' if sea_level > 0.8 else (
            'Low' if sea_level < 0.3 else 'Mid')
...
def get_spots_with_predictions():
    ...
    for i, spot in enumerate(SURF_SPOTS, 1):
        try:
            ...
            if i % 10 == 0:
                print(
                    f"Processed {i}/{len(SURF_SPOTS)} spots", file=sys.stderr)

        except Exception as e:
            print(
                f"Error processing spot {spot.get('name', 'unknown')}: {e}", file=sys.stderr)
    ...
    print(
        f"Successfully generated data for {len(all_spots_data)} spots", file=sys.stderr)
