...
function calculatePoseStats(progressData) {
  ...
  const completedDrills = Object.keys(drills).filter(drillId => drills[drillId]?.completed > 0);
  const scores = {};
...
function calculateARStats(progressData) {
  ...
  const completedModules = Object.keys(modules).filter(moduleId => modules[moduleId]?.completed > 0);
      ...
      >
        ...
        >
          <View style={styles.tabsContainer}>
            ...
            >
              <Icon name="fitness-center" size={18} color={activeTab === 'stats' ? '#fff' : '#666'} />
              <Text style={[styles.tabText, activeTab === 'stats' && styles.tabTextActive]}>Cardio</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.tab, activeTab === 'pose' && styles.tabActive]}
              onPress={() => setActiveTab('pose')}
            >
              <Icon name="accessibility" size={18} color={activeTab === 'pose' ? '#fff' : '#666'} />
              <Text style={[styles.tabText, activeTab === 'pose' && styles.tabTextActive]}>Pose</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.tab, activeTab === 'ar' && styles.tabActive]}
              onPress={() => setActiveTab('ar')}
            >
              <Icon name="view-in-ar" size={18} color={activeTab === 'ar' ? '#fff' : '#666'} />
              <Text style={[styles.tabText, activeTab === 'ar' && styles.tabTextActive]}>AR</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.tab, activeTab === 'badges' && styles.tabActive]}
              onPress={() => setActiveTab('badges')}
            >
              <Icon name="emoji-events" size={18} color={activeTab === 'badges' ? '#fff' : '#666'} />
            ...
            >
              <Icon name="history" size={18} color={activeTab === 'history' ? '#fff' : '#666'} />
            ...
            <View style={styles.section}>
              ...
              <View style={styles.recordCard}>
                ...
                <View style={styles.recordContent}>
                  ...
                  <Text style={styles.recordValue}>{stats.longestStreak} days</Text>
                </View>
              </View>
              <View style={styles.recordCard}>
                ...
                <View style={styles.recordContent}>
                  ...
                  <Text style={styles.recordValue}>{stats.longestSingleWorkout} min</Text>
                </View>
              </View>
            </View>
          </>
        ...
        {activeTab === 'pose' && (
          <>
            <View style={styles.statsContainer}>
              ...
              <View style={styles.statCard}>
                <Icon name="schedule" size={32} color="#4CAF50" />
                <Text style={styles.statValue}>{formatTime(Math.floor(poseStatsData.totalTime / 60))}</Text>
                <Text style={styles.statLabel}>Total Time</Text>
              </View>
              <View style={styles.statCard}>
                <Icon name="check-circle" size={32} color="#FF6B4A" />
                <Text style={styles.statValue}>{poseStatsData.completedDrills.length}</Text>
            ...
            {poseStatsData.completedDrills.length > 0 && (
              <View style={styles.section}>
                ...
                {poseStatsData.completedDrills.map((drillId, idx) => {
                  ...
                  const bestScore = drillScores.length > 0 ? Math.max(...drillScores) : 0;
                  return (
                    <View key={idx} style={styles.recordCard}>
                      <Icon name="check-circle" size={24} color="#4CAF50" />
                      <View style={styles.recordContent}>
                        <Text style={styles.recordLabel}>{drillId}</Text>
                        <Text style={styles.recordValue}>Best: {bestScore.toFixed(0)}%</Text>
                      </View>
                    </View>
                  );
                })}
            ...
            {poseStatsData.completedDrills.length === 0 && (
              <View style={styles.section}>
                <Text style={styles.emptyText}>No pose practice sessions yet. Start practicing!</Text>
              </View>
            )}
          </>
        )}
        ...
        {activeTab === 'ar' && (
          <>
            <View style={styles.statsContainer}>
              ...
              <View style={styles.statCard}>
                <Icon name="schedule" size={32} color="#4CAF50" />
                <Text style={styles.statValue}>{formatTime(Math.floor(arStatsData.totalTime / 60))}</Text>
                <Text style={styles.statLabel}>Total Time</Text>
              </View>
              <View style={styles.statCard}>
                <Icon name="explore" size={32} color="#FF6B4A" />
                <Text style={styles.statValue}>{arStatsData.completedModules.length}</Text>
                <Text style={styles.statLabel}>Modules</Text>
              </View>
              <View style={styles.statCard}>
                <Icon name="star" size={32} color="#FF5722" />
                <Text style={styles.statValue}>{arFormatted.accuracy || '0%'}</Text>
                <Text style={styles.statLabel}>Accuracy</Text>
              </View>
            </View>
            ...
            {arStatsData.completedModules.length === 0 && (
              <View style={styles.section}>
                <Text style={styles.emptyText}>No AR sessions yet. Start exploring!</Text>
              </View>
            )}
          </>
        )}
          ...
          <>
            <View style={styles.badgeSummary}>
              <Text style={styles.badgeSummaryText}>
                🏆 {earnedBadges.length + poseEarnedBadges.length + arEarnedBadges.length} Total Badges Earned
              </Text>
            </View>
            ...
            <View style={styles.section}>
              ...
              {['bronze', 'silver', 'gold', 'platinum', 'diamond'].map(tier => {
                const tierBadges = getBadgesByCategory('cardio').filter(b => b.tier === tier);
                const earnedInTier = tierBadges.filter(b => earnedBadges.includes(b.id));
                ...
                return (
                  <View key={tier} style={styles.tierSection}>
                    <Text style={styles.tierTitle}>
                      {tier.charAt(0).toUpperCase() + tier.slice(1)} Tier ({earnedInTier.length}/{tierBadges.length})
                    </Text>
            ...
            <View style={styles.section}>
              ...
              {['bronze', 'silver', 'gold', 'platinum', 'diamond'].map(tier => {
                const tierBadges = getBadgesByCategory('poseEstimation').filter(b => b.tier === tier);
                const earnedInTier = tierBadges.filter(b => poseEarnedBadges.includes(b.id));
            ...
            <View style={styles.section}>
              ...
              {['bronze', 'silver', 'gold', 'platinum', 'diamond'].map(tier => {
                const tierBadges = getBadgesByCategory('ar').filter(b => b.tier === tier);
                const earnedInTier = tierBadges.filter(b => arEarnedBadges.includes(b.id));
        ...
        {activeTab === 'history' && (
          <View style={styles.section}>
            ...
            {cardioWorkouts.length === 0 ? (
              ...
              cardioWorkouts.slice(-10).reverse().map((workout, index) => (
                <View key={index} style={styles.historyCard}>
                  <View style={styles.historyHeader}>
                    ...
                    <View style={[
                      ...
                      { backgroundColor: workout.completionRate >= 80 ? '#4CAF50' : workout.completionRate >= 50 ? '#FF9500' : '#FF3B30' }
                    ]}>
                      <Text style={styles.completionText}>{workout.completionRate}%</Text>
                    </View>
                  </View>
                  <Text style={styles.historyPlan}>{workout.planName}</Text>
                  <View style={styles.historyStats}>
                    <Text style={styles.historyStatItem}>
                      <Icon name="schedule" size={14} color="#666" /> {workout.totalDurationActual || 0}min
                    </Text>
                    <Text style={styles.historyStatItem}>
                      <Icon name="check-circle" size={14} color="#4CAF50" /> {workout.activitiesCompleted}
                    </Text>
                    <Text style={styles.historyStatItem}>
                      <Icon name="cancel" size={14} color="#FF3B30" /> {workout.activitiesSkipped}
                    </Text>
                  </View>
                </View>
              ))
        ...
        {!isEarned && progress && (
          <>
            ...
            <Text style={styles.progressText}>
              {progress.currentValue} / {badge.requirement} {badge.requirementUnit} 
...
const styles = StyleSheet.create({
  ...
  tabsContainer: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 12, padding: 4, marginBottom: 20, minWidth: '100%' },
  tab: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, paddingHorizontal: 8, borderRadius: 8, gap: 6, minWidth: 80 },
  tabActive: { backgroundColor: '#667eea' },
  tabText: { fontSize: 13, fontWeight: '600', color: '#666' },
  tabTextActive: { color: '#fff' },
  
  statsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginBottom: 24 },
  statCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, alignItems: 'center', flex: 1, minWidth: '45%', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  ...
  badgeSummary: { backgroundColor: '#007AFF', padding: 16, borderRadius: 12, marginBottom: 20 },
  badgeSummaryText: { fontSize: 18, fontWeight: 'bold', color: '#fff', textAlign: 'center' },
  
  tierSection: { marginBottom: 24 },
  tierTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 12 },
  
  badgeCard: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
  badgeCardEarned: { borderWidth: 2, borderColor: '#4CAF50' },
  badgeCardNext: { backgroundColor: '#E3F2FD', borderWidth: 2, borderColor: '#007AFF' },
  ...
  progressBarContainer: { height: 8, backgroundColor: '#e0e0e0', borderRadius: 4, overflow: 'hidden', marginBottom: 8 },
  progressBarFill: { height: '100%', borderRadius: 4 },
  progressText: { fontSize: 12, color: '#666' },
  earnedText: { fontSize: 14, color: '#4CAF50', fontWeight: '600' },
  
  recordCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 16, borderRadius: 12, marginBottom: 12 },
  ...
  completionBadge: { paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12 },
  completionText: { fontSize: 12, color: '#fff', fontWeight: 'bold' },
  historyPlan: { fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 8 },
  historyStats: { flexDirection: 'row', gap: 16 },
  historyStatItem: { fontSize: 12, color: '#666' },
  
  emptyText: { fontSize: 14, color: '#999', fontStyle: 'italic', textAlign: 'center', marginTop: 20 },
});
