...
const ForecastChart = ({ spotId = "2", forecast = null, onViewModeChange }) => {
  ...
  // Update forecastData when forecast prop changes
  useEffect(() => {
    if (forecast) {
  ...
  useEffect(() => {
    // Only fetch if forecast prop is not provided
    if (forecast) return;
        ...
        {forecastData.metadata && (
          <View style={styles.metadataContainer}>
            <Text style={styles.metadataText}>
              📊{" "}
              {forecastData.metadata.dataSource === "Mock"
                ? "Demo Forecast"
                : "AI-Powered Forecast"}
              {forecastData.metadata.forecastMethod &&
                ` • ${forecastData.metadata.forecastMethod}`}
            </Text>
            {viewMode === "hourly" && (
              <Text style={styles.metadataText}>
                🕐 Showing 24-hour forecast for{" "}
                {forecastData.labels[selectedDay]}
              </Text>
            )}
          </View>
        )}
