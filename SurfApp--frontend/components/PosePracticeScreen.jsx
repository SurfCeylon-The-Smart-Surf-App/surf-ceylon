...
import {
  ...
  PoseLandmarks,
  PoseLandmark,
  isPersonDetectedProgressive,
} from "../utils/poseDetection.js";
import {
  smoothLandmarks,
  clearSmoothingBuffers,
} from "../utils/landmarkSmoothing.js";
import {
...
const drills = [
  {
    ...
    instruction:
      "Keep knees bent (90-140 deg), hips hinged (140-170 deg), and maintain balance",
  ...
  },
  {
    ...
    instruction:
      "Lie down, push up, then jump to your feet in one fluid motion",
    icon: "arrow-upward",
    color: "#007AFF",
    key: "2",
  },
  {
    ...
    instruction:
      "Keep your back arched (less than 165 deg), head up, and look forward while paddling",
    icon: "rowing",
    color: "#5AC8FA",
    key: "3",
  },
  {
    ...
    instruction:
      "Bend knees deep (less than 120 deg), rotate shoulders more than hips",
    icon: "rotate-right",
    color: "#FF9500",
    key: "4",
  },
  {
    ...
    instruction:
      "Compress down (less than 110 deg), then extend up (more than 140 deg) in rhythmic motion",
    icon: "trending-up",
    color: "#FF2D55",
    key: "5",
  },
  {
    ...
    instruction:
      "Get LOW! Bend knees (less than 90 deg) and hips (less than 100 deg) for deep crouch",
    icon: "waves",
    color: "#AF52DE",
    key: "6",
  },
  ...
  },
  {
    ...
    instruction:
      "Turn head and shoulders first, keep stance balanced, lead with your head",
    icon: "swap-horiz",
    color: "#FFCC00",
    key: "8",
  },
...
export default function PosePracticeScreen() {
  ...
  const processPoseFromFrame = async (base64Image) => {
    ...
    try {
      ...
      try {
        ...
        const sessionId = practiceStartTime
          ? `session_${practiceStartTime}`
          : `session_${Date.now()}`;
        ...
        const {
          ...
          addVariation,
        } = require("../utils/mockPoseDetector.js");
      ...
      if (detectionResult.stage !== "none" && smoothedLandmarks) {
        ...
        setPersonDetected(
          detectionResult.stage === "preview" ||
            detectionResult.stage === "partial" ||
            detectionResult.stage === "full"
        );
      } else {
      ...
      const isReadyForAction =
        (calibrationStatus === "ready" ||
          calibrationStatus === "positioning") && // Allow positioning stage
      ...
      if (isReadyForAction) {
            ...
            .map((r) => ({
              ...
              type:
                r.severity === "error"
                  ...
                  : "info",
              priority:
                r.severity === "error" ? 3 : r.severity === "warning" ? 2 : 1,
            }));
        ...
        if (analysis.personDetected) {
          ...
          humanFramesDetectedRef.current = Math.max(
            0,
            humanFramesDetectedRef.current - 1
        ...
        const confirmedPersonDetected =
          humanFramesDetectedRef.current >= REQUIRED_CONSECUTIVE_FRAMES &&
        ...
        if (confirmedPersonDetected && analysis.personDetected) {
          ...
          const newScore = Math.min(
            100,
            Math.max(0, Math.round(analysis.score))
          );
          setScore(newScore);
          ...
          if (selectedDrill) {
            setDrillStats((prev) => {
              const stats = prev[selectedDrill] || {
                correctTime: 0,
                totalTime: 0,
                reps: 0,
                bestScore: 0,
              ...
              return {
                ...
                [selectedDrill]: {
                  ...
                  correctTime:
                    newScore > 70 ? stats.correctTime + 100 : stats.correctTime,
                },
              };
            });
          }
          ...
          const avg =
            recentScores.length > 0
              ? recentScores.reduce((sum, s) => sum + s, 0) /
                recentScores.length
              : newScore;
          ...
          if (selectedDrill === "pumping") {
            previousDataRef.current.pumpState =
              analysis.score > 70 ? "HIGH" : "LOW";
        ...
        if (detectionResult.stage === "preview") {
          setFeedback([
            {
              text: `Detecting... (${Math.round(
                detectionResult.confidence
              )}% confidence). Step fully into view.`,
              type: "info",
              priority: 1,
            },
          ]);
          ...
          setFeedback([
            {
              text: `Almost there! (${Math.round(
                detectionResult.confidence
              )}% confidence). Show more of your body.`,
              type: "warning",
              priority: 1,
            },
          ]);
          ...
          humanFramesDetectedRef.current > 0 &&
          humanFramesDetectedRef.current < 2
        ) {
          setFeedback([
            {
  ...
  const handleStartPractice = (drillId) => {
    if (!hasPermission) {
      Alert.alert(
        "Permission Required",
        "Camera permission is required for pose practice"
      );
      return;
    }
  ...
  const handleStopPractice = async () => {
    ...
    if (selectedDrill) {
      const stats = drillStats[selectedDrill] || {
        correctTime: 0,
        totalTime: 0,
        reps: 0,
        bestScore: 0,
      ...
      const accuracy =
        stats.totalTime > 0 ? (stats.correctTime / stats.totalTime) * 100 : 0;
      ...
      try {
        ...
        let userProgress = {
          pose: { level: 1, xp: 0, xpToNext: 100 },
          poseEstimation: { level: 1, xp: 0, xpToNext: 100 },
        };
        try {
        ...
        const poseProgress = userProgress.pose ||
          userProgress.poseEstimation || { level: 1, xp: 0, xpToNext: 100 };
        const xpProgression = calculateXPProgression(
          poseProgress.xp || 0,
          poseProgress.level || 1,
          sessionXP
        );
        ...
        const fullProgress =
          userProgress.pose || userProgress.poseEstimation || {};
        const badgeCheckProgress = {
          completedDrills: [
            ...(fullProgress.completedDrills || []),
            ...(stats.reps > 0 ? [selectedDrill] : []),
          ],
          scores: {
            ...(fullProgress.scores || {}),
            [selectedDrill]: [
              ...(fullProgress.scores?.[selectedDrill] || []),
              stats.bestScore,
            ],
          },
        ...
        const newBadges = checkPoseBadges(
          badgeCheckProgress,
          badgeCheckProgress.badges
        );
        ...
        if (newBadges.length > 0) {
          ...
          Alert.alert(
            "Badge Earned!",
            `Congratulations! You've earned ${newBadges.length} new badge${
  ...
  const renderSkeleton = (landmarks, stage) => {
    ...
    if (landmarks.nose) {
      if (landmarks.leftEye)
        skeletonLines.push(
          createLine(landmarks.nose, landmarks.leftEye, "nose-leftEye")
        );
      if (landmarks.rightEye)
        skeletonLines.push(
          createLine(landmarks.nose, landmarks.rightEye, "nose-rightEye")
        );
      if (landmarks.leftEar && landmarks.leftEye)
        skeletonLines.push(
          createLine(landmarks.leftEye, landmarks.leftEar, "leftEye-leftEar")
        );
      if (landmarks.rightEar && landmarks.rightEye)
        skeletonLines.push(
          createLine(
            landmarks.rightEye,
            landmarks.rightEar,
    ...
    if (landmarks.leftShoulder && landmarks.rightShoulder) {
      skeletonLines.push(
        createLine(landmarks.leftShoulder, landmarks.rightShoulder, "shoulders")
      );
    }
    ...
    if (landmarks.leftHip && landmarks.rightHip) {
      skeletonLines.push(
        createLine(landmarks.leftHip, landmarks.rightHip, "hips")
      );
    }
    ...
    if (landmarks.leftHip && landmarks.leftKnee) {
      skeletonLines.push(
        createLine(landmarks.leftHip, landmarks.leftKnee, "leftHip-leftKnee")
      );
    }
    ...
    // Joints
    if (landmarks.nose)
      skeletonJoints.push(createJoint(landmarks.nose, "joint-nose"));
    if (landmarks.leftShoulder)
      skeletonJoints.push(
        createJoint(landmarks.leftShoulder, "joint-leftShoulder")
      );
    if (landmarks.rightShoulder)
      skeletonJoints.push(
        createJoint(landmarks.rightShoulder, "joint-rightShoulder")
      );
    if (landmarks.leftElbow)
      skeletonJoints.push(createJoint(landmarks.leftElbow, "joint-leftElbow"));
    if (landmarks.rightElbow)
      skeletonJoints.push(
        createJoint(landmarks.rightElbow, "joint-rightElbow")
      );
    if (landmarks.leftWrist)
      skeletonJoints.push(createJoint(landmarks.leftWrist, "joint-leftWrist"));
    if (landmarks.rightWrist)
      skeletonJoints.push(
        createJoint(landmarks.rightWrist, "joint-rightWrist")
      );
    if (landmarks.leftHip)
      skeletonJoints.push(createJoint(landmarks.leftHip, "joint-leftHip"));
    if (landmarks.rightHip)
      skeletonJoints.push(createJoint(landmarks.rightHip, "joint-rightHip"));
    if (landmarks.leftKnee)
      skeletonJoints.push(createJoint(landmarks.leftKnee, "joint-leftKnee"));
    if (landmarks.rightKnee)
      skeletonJoints.push(createJoint(landmarks.rightKnee, "joint-rightKnee"));
    if (landmarks.leftAnkle)
      skeletonJoints.push(createJoint(landmarks.leftAnkle, "joint-leftAnkle"));
    if (landmarks.rightAnkle)
      skeletonJoints.push(
        createJoint(landmarks.rightAnkle, "joint-rightAnkle")
      );
        ...
        <View style={styles.menuContainer}>
          ...
          >
            {drills.map((drill, index) => {
              const stats = drillStats[drill.id] || {
                correctTime: 0,
                totalTime: 0,
                reps: 0,
                bestScore: 0,
              ...
              const accuracy =
                stats.totalTime > 0
                  ? (stats.correctTime / stats.totalTime) * 100
                  : 0;
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.drillCard,
                    { borderLeftColor: drill.color, borderLeftWidth: 6 },
                  ]}
                  ...
                  <View style={styles.drillCardHeader}>
                    <View
                      style={[
                        ...
                        { backgroundColor: drill.color + "20" },
                      ]}
                    >
                    ...
                    <View style={styles.drillCardInfo}>
                      ...
                      <Text style={styles.drillCardDescription}>
                        {drill.description}
                      </Text>
                      <Text style={styles.drillCardInstruction}>
                        {drill.instruction}
                      </Text>
                    </View>
                  </View>
                    ...
                    <View style={styles.drillStatsRow}>
                      <View style={styles.statItem}>
                        ...
                        <Text style={styles.statText}>
                          Best: {stats.bestScore.toFixed(0)}%
                        </Text>
                      </View>
                      <View style={styles.statItem}>
          ...
          <View style={styles.menuFooter}>
            <Text style={styles.footerText}>
              Tip: Use the side panel buttons (1-8) to quickly switch between
        ...
        <View style={styles.cameraContainer}>
          {isExpoGo ? (
            ...
            >
              ...
              >
                To use pose detection features, you need to build a development
                build:{"\n\n"}
            ...
            <>
              ...
              <View style={styles.overlayTop}>
                ...
                <View style={styles.statusBar}>
                  ...
                  >
                    <Icon
                      name={showSkeleton ? "visibility" : "visibility-off"}
                      size={16}
                      color="#fff"
                    />
                  </TouchableOpacity>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setShowReferenceSkeleton(!showReferenceSkeleton)
                    }
                  >
                    <Icon
                      name={
                  ...
                  {isRecording && (
                    ...
                    >
                      ...
                      <Text style={styles.timerText}>
                        {Math.floor(drillTimer / 60)}:
                        {(drillTimer % 60).toString().padStart(2, "0")}
                      </Text>
                    </View>
                  )}
                    ...
                    <View style={styles.scoreGroupContainer}>
                      <View
                        style={[
                          ...
                          { backgroundColor: getScoreColor() + "CC" },
                        ]}
                      >
                        <AnimatedText
                          style={[
                  ...
                  <View style={styles.detectionStatusContainer}>
                    ...
                    <Text style={styles.detectionStatusText}>
                      {detectionStage === "full"
                        ? "✓ Ready"
                        : detectionStage === "partial"
                        ? `Positioning... ${Math.round(detectionConfidence)}%`
                        : detectionStage === "preview"
                        ? `Detecting ${Math.round(detectionConfidence)}%`
                        : "Waiting..."}
                    </Text>
                  </View>
                )}
                ...
                {isRecording &&
                  ...
                  detectionStage !== "none" && (
                    <View style={styles.angleArcsOverlay} pointerEvents="none">
                      {/* Knee angle arcs */}
                      {currentLandmarks.leftHip &&
                        currentLandmarks.leftKnee &&
                        currentLandmarks.leftAnkle &&
                        currentBodyMetrics.kneeAngles.left > 0 && (
                          <AngleArcOverlay
                            ...
                            targetAngle={
                              selectedDrill === "stance"
                                ? 115
                                : selectedDrill === "tube_stance"
                                ? 90
                                : selectedDrill === "bottom_turn"
                                ? 100
                                : 115
                            }
                            targetRange={
                      ...
                      {currentLandmarks.rightHip &&
                        currentLandmarks.rightKnee &&
                        currentLandmarks.rightAnkle &&
                        currentBodyMetrics.kneeAngles.right > 0 && (
                          <AngleArcOverlay
                      ...
                      {selectedDrill === "stance" &&
                        currentLandmarks.leftShoulder &&
                        currentLandmarks.leftHip &&
                        currentLandmarks.leftKnee &&
                        currentBodyMetrics.hipAngles.left > 0 && (
                          <AngleArcOverlay
                      ...
                      {selectedDrill === "stance" &&
                        currentLandmarks.rightShoulder &&
                        currentLandmarks.rightHip &&
                        currentLandmarks.rightKnee &&
                        currentBodyMetrics.hipAngles.right > 0 && (
                          <AngleArcOverlay
                ...
                {isRecording && personDetected && (
                  <View
                    style={styles.liveScoreBarContainer}
                    pointerEvents="none"
                  >
                    ...
                    {drills.map((drill) => (
                      ...
                      >
                        ...
                        <Icon
                          ...
                          size={20}
                          color={
                            selectedDrill === drill.id ? "#fff" : drill.color
                          }
                        />
                      </TouchableOpacity>
                    ))}
