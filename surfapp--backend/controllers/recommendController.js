...
const getRecommendation = asyncHandler(async (req, res) => {
  ...
  if (!skillLevel || !goal || (Array.isArray(goal) && goal.length === 0)) {
    return res
      .status(400)
      .json({ error: "Missing required fields: skillLevel and goal" });
  }
  ...
  const normalizedGoals = goalArray
    .map((g) => GOAL_MAP[normalize(g)] || g)
    .filter(Boolean);

  console.log("[recommend] incoming", {
    skillLevel,
    goal,
    userId,
    userDetails,
  ...
  console.log("[recommend] normalized", {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      try {
        ...
        json = {
          error: "Failed to parse model server response",
          status: resp.status,
        };
      }
      ...
      const errorPayload = {
        error:
          (json &&
            (json.error ||
              (json.detail && (json.detail.error || json.detail.message)) ||
              json.message)) ||
          "Model server error",
        details:
          (json &&
            (json.details ||
              (json.detail && json.detail.details) ||
              json.detail)) ||
          `HTTP ${resp.status}`,
        status: resp.status,
      };
    ...
    if (firebaseAdmin.isInitialized() && userId) {
      try {
        ...
        console.error(
          "[recommend] Failed to save plan to Firestore",
          e.message || e
        );
      }
    }
