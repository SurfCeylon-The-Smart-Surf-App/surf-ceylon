...
const ML_CARDIO_SERVER_URL = process.env.ML_CARDIO_SERVER_URL || "http://127.0.0.1:5001/api/ai-tutor/recommend";
...
const getRecommendation = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const workoutPlan = await resp.json();
    console.log("[recommend] ML server response received successfully");
    console.log("[recommend] Generated plan:", workoutPlan.planName);
    console.log("[recommend] Exercises:", workoutPlan.exercises?.length || 0);

    // Optionally persist plan to Firestore
    if (firebaseAdmin.isInitialized() && userId) {
      try {
        ...
        console.log("[recommend] Plan saved to Firestore");
      } catch (e) {
        console.error("[recommend] Failed to save plan to Firestore:", e.message);
      }
    }
    ...
    const responsePayload = {
      ...
      ...workoutPlan // Also include top-level fields for backward compatibility
    };

    console.log("[recommend] Sending response with recommendedPlans wrapper");
