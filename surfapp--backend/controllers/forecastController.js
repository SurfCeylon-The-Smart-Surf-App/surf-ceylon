...
const getForecastChart = async (req, res) => {
  try {
    ...
    if (spotName) {
      // Find by name
      spot = spotsData.find(
        (s) => s.name.toLowerCase() === spotName.toLowerCase()
      );
    } else if (spotId) {
      // Find by ID
      spot = spotsData.find((s) => s.id === spotId);
    } else {
      // Default to Weligama
      spot = spotsData.find((s) => s.id === "2");
    }

    if (!spot || !spot.coords || spot.coords.length !== 2) {
      return res
        .status(404)
        .json({ error: "Spot not found or invalid coordinates" });
    }
    ...
    const pythonProcess = spawn(
      PYTHON_EXECUTABLE,
      [FORECAST_7DAY_SCRIPT, lat.toString(), lng.toString()],
      {
    ...
    pythonProcess.stdout.on(
      "data",
      (data) => (pythonOutput += data.toString())
    );
      ...
      try {
        ...
        if (forecastData.daily && forecastData.hourly) {
          ...
          res.json({
            labels: forecastData.labels || generateDateLabels(),
            waveHeight:
              forecastData.forecast?.waveHeight || forecastData.waveHeight,
            wavePeriod:
              forecastData.forecast?.wavePeriod || forecastData.wavePeriod,
            swellHeight:
              forecastData.forecast?.swellHeight || forecastData.swellHeight,
            swellPeriod:
              forecastData.forecast?.swellPeriod || forecastData.swellPeriod,
            windSpeed:
              forecastData.forecast?.windSpeed || forecastData.windSpeed,
            windDirection:
              forecastData.forecast?.windDirection ||
