...
const { POSE_SERVER_URL, POSE_SERVER_TIMEOUT } = require('../config/aiConstants');
...
const detectPose = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.log('[pose] Pose detection successful, person detected:', result.personDetected);
    ...
    if (error.code === 'ECONNREFUSED' || error.message.includes('ECONNREFUSED')) {
      ...
      errorDetails = 'The pose detection server took too long to respond. Please try again.';
    }
